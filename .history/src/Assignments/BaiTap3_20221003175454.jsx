...
const BaiTap3 = () => {
  ...
  const handleSlide = () => {
    ...
    if (listUploadedRef.current && fileUploadedRef.current) {
      let scrollTo = 0;
      const fileUploadedStyleCSs = 2 + 8 + 8
      const fileLength = files.length
      if (
        listUploadedOffsetWidth + listUploadedScroll <
        (fileUploadedWidth + fileUploadedStyleCSs) * fileLength
      ) {
        scrollTo = listUploadedScroll + fileUploadedWidth + 18;
      } else {
        scrollTo = (fileUploadedWidth + 18) * fileLength;
      }
